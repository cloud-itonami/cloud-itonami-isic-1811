(ns printing.phase
  "Phase gate for the printing-plant operations coordinator rollout
  (0->3 maturity). Mirrors `tobaccoops.phase` (cloud-itonami-isic-0115) in
  shape, adapted to this actor's op set (no separate request-level
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`printing.governor/always-escalate-ops`) escalate even when clean;
  ...
  This namespace did not exist in a prior attempt at this repo -- there
  was no rollout-phase gate at all, only a hand-rolled `evaluate` that
  returned `:holds?`/`:clean?` with no path to an actual commit/hold/
  escalate disposition or graph. See `operation.cljc` for how the phase
  gate is invoked."
  (:require [printing.governor :as governor]))
